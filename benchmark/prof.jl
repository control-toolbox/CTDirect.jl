...
function init(; in_place, grid_size, disc_method)
    ...
    docp = CTDirect.DOCP(
        ...
        disc_method=string(disc_method),
    )
...
function test_unit(;
    ...
    if test_block
        ...
        warntype &&
            @code_warntype CTDirect.setConstraintBlock!(docp, c, xu, v, times, i, work)
        jet && display(
            @report_opt CTDirect.setConstraintBlock!(docp, c, xu, v, times, i, work)
        )
        if profile
            Profile.Allocs.@profile sample_rate = 1.0 CTDirect.setConstraintBlock!(
                docp, c, xu, v, times, i, work
            )
            results = Profile.Allocs.fetch()
            PProf.Allocs.pprof()
        end
    ...
    if test_cons
        ...
        if profile
            Profile.Allocs.@profile sample_rate = 1.0 CTDirect.DOCP_constraints!(
                c, xu, docp
            )
            results = Profile.Allocs.fetch()
            PProf.Allocs.pprof()
