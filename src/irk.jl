...
# +++ remove calls for control at tf !
...
function IRK_dims(dim_NLP_steps, dim_NLP_x, dim_NLP_u, dim_NLP_v, dim_u_cons, dim_x_cons, dim_xu_cons, dim_boundary_cons, dim_v_cons, stage; control_disc=:step)
    ...
    dim_NLP_constraints = dim_NLP_steps * (dim_NLP_x + (dim_NLP_x * stage) + step_pathcons_block) + dim_boundary_cons + dim_v_cons
...
struct Midpoint_IRK <: GenericIRK
    ...
    function Midpoint_IRK(dim_NLP_steps, dim_NLP_x, dim_NLP_u, dim_NLP_v, dim_u_cons, dim_x_cons, dim_xu_cons, dim_boundary_cons, dim_v_cons)
        ...
        disc = new(false, stage, hcat(0.5), [1], [0.5], step_variables_block, step_pathcons_block, :step, "Implicit Midpoint aka Gauss-Legendre collocation for s=1, 2nd order, symplectic")
...
function get_stagevars_at_time_step(xu, docp::DOCP{ <: GenericIRK}, i, j)
    if docp.discretization.control_disc == :step
        ...
        offset = (i-1) * docp.discretization._step_variables_block + docp.dim_NLP_x + docp.dim_NLP_u * docp.discretization.stage + (j-1)*docp.dim_NLP_x
    end
...
function get_stage_grid(docp::DOCP{ <: GenericIRK}, time_grid)

    s = docp.discretization.stage
    stage_grid = zeros(docp.dim_NLP_steps * s)
    ...
    for i=1:docp.dim_NLP_steps
        ...
        hi = tip1 - ti
        for j=1:s
            stage_grid[(i-1)*s+j] = ti + docp.discretization.butcher_c[j] * hi
        end
    end
...
"""
function setPathBounds!(docp::DOCP{ <: GenericIRK}, index::Int, lb, ub)
