...
function ipopt_constraint(xu, ctd)
    ...
    v::eltype(xu)  = get_variable(xu, ctd)

    # state equation
    ti = t0
    xi::Vector{eltype(xu)} = get_state_at_time_step(xu, ctd, 0)
    ui::eltype(xu) = get_control_at_time_step(xu, ctd, 0)
    fi::Vector{eltype(xu)}  = ctd.dynamics(ti, xi, ui, v)
    xip1::Vector{eltype(xu)} = get_state_at_time_step(xu, ctd, 0)
    uip1::eltype(xu) = get_control_at_time_step(xu, ctd, 0)
    fip1::Vector{eltype(xu)}  = ctd.dynamics(ti, xi, ui, v)
    if ctd.has_lagrange_cost
        xli::eltype(xu)  = get_lagrange_cost_at_time_step(xu, ctd, 0)
        li::eltype(xu)  = ctd.lagrange(ti, xi, ui, v)
    end
    ...
    for i in 0:N-1
        ...
        if ctd.has_lagrange_cost
            xlip1::eltype(xu)  = get_lagrange_cost_at_time_step(xu, ctd, i+1)
            lip1::eltype(xu)  = ctd.lagrange(tip1, xip1, uip1, v)
    ...
    # path constraints at final time
    xf::Vector{eltype(xu)}  = get_state_at_time_step(xu, ctd, N)
    uf::eltype(xu) = get_control_at_time_step(xu, ctd, N)
    if ctd.has_control_constraints
    ...
    # boundary conditions
    if ctd.has_boundary_conditions
        x0::Vector{eltype(xu)}  = get_state_at_time_step(xu, ctd, 0)
