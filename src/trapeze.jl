...
function get_OCP_variable(
    xu, docp::DOCP{<:Discretization,<:ScalVect,<:ScalVect,ScalVariable}
)
    return xu[docp.dim_NLP_variables]
end
function get_OCP_variable(
    xu, docp::DOCP{<:Discretization,<:ScalVect,<:ScalVect,VectVariable}
)
...
function get_OCP_state_at_time_step(
    xu, docp::DOCP{Trapeze,ScalVariable,<:ScalVect,<:ScalVect}, i
)
    offset = (i - 1) * (docp.dim_NLP_x + docp.dim_NLP_u)
    return xu[offset + 1]
end
function get_OCP_state_at_time_step(
    xu, docp::DOCP{Trapeze,VectVariable,<:ScalVect,<:ScalVect}, i
)
    offset = (i - 1) * (docp.dim_NLP_x + docp.dim_NLP_u)
...
function get_OCP_control_at_time_step(
    xu, docp::DOCP{Trapeze,<:ScalVect,ScalVariable,<:ScalVect}, i
)
    offset = (i - 1) * (docp.dim_NLP_x + docp.dim_NLP_u) + docp.dim_NLP_x
    return xu[offset + 1]
end
function get_OCP_control_at_time_step(
    xu, docp::DOCP{Trapeze,<:ScalVect,VectVariable,<:ScalVect}, i
)
    offset = (i - 1) * (docp.dim_NLP_x + docp.dim_NLP_u) + docp.dim_NLP_x
...
function setWorkArray(docp::DOCP{Trapeze}, xu, time_grid, v)
    ...
    if docp.is_lagrange
        if docp.is_inplace
            docp.ocp.lagrange(
                (@view work[(docp.dim_NLP_x):(docp.dim_NLP_x)]), t0, x0, u0, v
            )
        else
...
function setConstraintBlock!(docp::DOCP{Trapeze}, c, xu, v, time_grid, i, work)
    ...
    if i <= docp.dim_NLP_steps
        ...
        if docp.is_lagrange
            if docp.is_inplace
                docp.ocp.lagrange(
                    (@view work[(docp.dim_NLP_x):(docp.dim_NLP_x)]), tip1, xip1, uip1, v
                )
            else
        ...
        if docp.dim_OCP_x == 1
            ...
            c[(offset + 1):(offset + docp.dim_OCP_x)] =
                xip1 - (
                    xi +
                    0.5 * (tip1 - ti) * (fi[1:(docp.dim_OCP_x)] + work[1:(docp.dim_OCP_x)])
                )
        end
        if docp.is_lagrange
            c[offset + docp.dim_NLP_x] =
                get_lagrange_state_at_time_step(xu, docp, i + 1) - (
                    ...
                    0.5 * (tip1 - ti) * (fi[docp.dim_NLP_x] + work[docp.dim_NLP_x])
                )
        end
    ...
    if docp.dim_u_cons > 0
        if docp.is_inplace
            docp.control_constraints[2](
                (@view c[(offset + 1):(offset + docp.dim_u_cons)]), ti, ui, v
            )
        else
