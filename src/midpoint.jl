...
struct Midpoint <: Discretization
    ...
    function Midpoint(dim_NLP_steps, dim_NLP_x, dim_NLP_u, dim_NLP_v, dim_u_cons, dim_x_cons, dim_xu_cons, dim_boundary_cons, dim_v_cons)
        ...
        dim_NLP_constraints = dim_NLP_steps * (dim_NLP_x + dim_NLP_x + step_pathcons_block) + dim_boundary_cons + dim_v_cons

        disc = new(false, 1, step_pathcons_block, :step, "Implicit Midpoint aka Gauss-Legendre collocation for s=1, 2nd order, symplectic")
