...
"""
function get_OCP_state_at_time_step(
    xu, docp::DOCP{Midpoint,ScalVariable,<:ScalVect,<:ScalVect}, i
)
    offset = (i - 1) * (docp.dim_NLP_x * (1 + docp.discretization.stage) + docp.dim_NLP_u)
    return xu[offset + 1]
end
function get_OCP_state_at_time_step(
    xu, docp::DOCP{Midpoint,VectVariable,<:ScalVect,<:ScalVect}, i
)
    offset = (i - 1) * (docp.dim_NLP_x * (1 + docp.discretization.stage) + docp.dim_NLP_u)
...
function get_OCP_control_at_time_step(
    xu, docp::DOCP{Midpoint,<:ScalVect,ScalVariable,<:ScalVect}, i
)
    if i < docp.dim_NLP_steps + 1
        offset =
            (i - 1) * (docp.dim_NLP_x * (1 + docp.discretization.stage) + docp.dim_NLP_u) +
            docp.dim_NLP_x
    else
        offset =
            (i - 2) * (docp.dim_NLP_x * (1 + docp.discretization.stage) + docp.dim_NLP_u) +
            docp.dim_NLP_x
    end
    return xu[offset + 1]
end
function get_OCP_control_at_time_step(
    xu, docp::DOCP{Midpoint,<:ScalVect,VectVariable,<:ScalVect}, i
)
    if i < docp.dim_NLP_steps + 1
        offset =
            (i - 1) * (docp.dim_NLP_x * (1 + docp.discretization.stage) + docp.dim_NLP_u) +
...
function setConstraintBlock!(docp::DOCP{Midpoint}, c, xu, v, time_grid, i, work)
    ...
    offset =
        (i - 1) * (docp.dim_NLP_x * (1 + docp.discretization.stage) + docp.dim_path_cons)
    ...
    if i <= docp.dim_NLP_steps
        ...
        if docp.dim_OCP_x == 1
            ...
            c[(offset + 1):(offset + docp.dim_OCP_x)] =
                xip1 - (xi + h_sum_bk[1:(docp.dim_OCP_x)])
        end
        if docp.is_lagrange
            c[offset + docp.dim_NLP_x] =
                get_lagrange_state_at_time_step(xu, docp, i + 1) -
                (get_lagrange_state_at_time_step(xu, docp, i) + h_sum_bk[docp.dim_NLP_x])
        end
        ...
        if docp.is_inplace
            docp.ocp.dynamics(
                (@view c[(offset + 1):(offset + docp.dim_OCP_x)]), ts, xs, ui, v
            )
            @views c[(offset + 1):(offset + docp.dim_OCP_x)] =
                -c[(offset + 1):(offset + docp.dim_OCP_x)] + ki[1:(docp.dim_OCP_x)]
        else
            c[(offset + 1):(offset + docp.dim_OCP_x)] .=
                ki[1:(docp.dim_OCP_x)] .- docp.ocp.dynamics(ts, xs, ui, v)
        end
        if docp.is_lagrange
            if docp.is_inplace
                docp.ocp.lagrange(
                    (@view c[(offset + docp.dim_NLP_x):(offset + docp.dim_NLP_x)]),
                    ts,
                    xs,
                    ui,
                    v,
                )
                @views c[offset + docp.dim_NLP_x] =
                    -c[offset + docp.dim_NLP_x] + ki[docp.dim_NLP_x]
            else
                c[offset + docp.dim_NLP_x] =
                    ki[docp.dim_NLP_x] - docp.ocp.lagrange(ts, xs, ui, v)
            end
        end
        offset += docp.dim_NLP_x
    end
    ...
    if docp.dim_u_cons > 0
        if docp.has_inplace
            docp.control_constraints[2](
                (@view c[(offset + 1):(offset + docp.dim_u_cons)]), ti, docp._u(ui), v
            )
        else
            c[(offset + 1):(offset + docp.dim_u_cons)] = docp.control_constraints[2](
                ti, docp._u(ui), v
            )
        end
    end
    if docp.dim_x_cons > 0
        if docp.has_inplace
            docp.state_constraints[2](
                (@view c[(offset + docp.dim_u_cons + 1):(offset + docp.dim_u_cons + docp.dim_x_cons)]),
                ti,
                docp._x(xi),
                v,
            )
        else
            c[(offset + docp.dim_u_cons + 1):(offset + docp.dim_u_cons + docp.dim_x_cons)] = docp.state_constraints[2](
                ti, docp._x(xi), v
            )
        end
    end
    if docp.dim_mixed_cons > 0
        if docp.has_inplace
            docp.mixed_constraints[2](
                (@view c[(offset + docp.dim_u_cons + docp.dim_x_cons + 1):(offset + docp.dim_u_cons + docp.dim_x_cons + docp.dim_mixed_cons)]),
                ti,
                docp._x(xi),
                docp._u(ui),
                v,
            ...
            c[(offset + docp.dim_u_cons + docp.dim_x_cons + 1):(offset + docp.dim_u_cons + docp.dim_x_cons + docp.dim_mixed_cons)] = docp.mixed_constraints[2](
                ti, docp._x(xi), docp._u(ui), v
            )
        end
    end
end
