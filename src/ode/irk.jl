...
"""
function stepStateConstraints!(docp::DOCP{<: GenericIRK}, c, xu, v, time_grid, i, work)
    ocp = ocp_model(docp)
    disc = disc_model(docp)
    dims = docp.dims
...
function DOCP_Jacobian_pattern(docp::DOCP{<: GenericIRK})
    ...
    for i in 1:docp.time.steps
        ...
        if docp.flags.lagrange
            ...
            for i in 1:s
                kij_l = var_offset + dims.NLP_x + dims.NLP_u + i*dims.NLP_x
...
function DOCP_Hessian_pattern(docp::DOCP{<: GenericIRK})
    ...
    for i in 1:docp.time.steps
        ...
        ui_end = var_offset + dims.NLP_x + dims.NLP_u
        ki_start = var_offset + dims.NLP_x + dims.NLP_u + 1
        ki_end = var_offset + (s+1)*dims.NLP_x + dims.NLP_u
