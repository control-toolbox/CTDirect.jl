...
function setWorkArray(docp::DOCP{Trapeze}, xu, time_grid, v)
    ...
    for i in 1:(docp.time.steps + 1)
        ...
        CTModels.dynamics(docp.ocp)(
            (@view work[(offset + 1):(offset + docp.dims.OCP_x)]), ti, xi, ui, v
        )
        # lagrange cost
    ...
    if i <= docp.time.steps
        ...
        @views @. c[(offset + 1):(offset + docp.dims.OCP_x)] =
            xip1 - (
                xi +
                half_hi * (
                    work[(offset_dyn_i + 1):(offset_dyn_i + docp.dims.OCP_x)] +
                    work[(offset_dyn_ip1 + 1):(offset_dyn_ip1 + docp.dims.OCP_x)]
                )
            )
        ...
        if docp.flags.lagrange && docp.flags.lagrange_to_mayer
            c[offset + docp.dims.NLP_x] =
                get_lagrange_state_at_time_step(xu, docp, i+1) - (
                    ...
                    half_hi * (
                        work[offset_dyn_i + docp.dims.NLP_x] +
                        work[offset_dyn_ip1 + docp.dims.NLP_x]
                    )
                )
        end
...
function DOCP_Jacobian_pattern(docp::DOCP{Trapeze})
    ...
    for i in 1:docp.time.steps
        ...
        add_nonzero_block!(
            Is, Js, c_offset+1, c_offset+docp.dims.OCP_x, uip1_start, uip1_end
        )
