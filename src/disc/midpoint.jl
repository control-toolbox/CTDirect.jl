...
"""
function get_OCP_state_at_time_step(xu, docp::DOCP{Midpoint}, i)
...
Scalar / Vector output
"""
function get_OCP_control_at_time_step(xu, docp::DOCP{Midpoint}, i)
    # final time case
...
function setStepConstraints!(docp::DOCP{Midpoint}, c, xu, v, time_grid, i, work)
    ...
    if i <= docp.dim_NLP_steps
        ...
        CTModels.dynamics(docp.ocp)((@view c[offset+1:offset+docp.dim_OCP_x]), ts, work, ui, v)
        if docp.has_lagrange
            c[offset+docp.dim_NLP_x] = CTModels.lagrange(docp.ocp)(ts, work, ui, v)
        end            
