...
function get_OCP_control_at_time_step(xu, docp::DOCP{Euler}, i)
    ...
    if disc._explicit
        ...
        offset = (i-1) * disc._step_variables_block + dims.NLP_x
    else
        # initial time case
        (i == 1) && (i = 2)
        offset = (i-2) * disc._step_variables_block + dims.NLP_x
    end
    return @view xu[(offset + 1):(offset + dims.NLP_u)]
end
...
function DOCP_Jacobian_pattern(docp::DOCP{Euler})
    ...
    for i in 1:docp.time.steps
        ...
        if docp.flags.lagrange
            if disc._explicit
                # [x_i, l_i, u_i]
                add_nonzero_block!(Is, Js, dyn_lag, dyn_lag, xi_start, ui_end)
                # l_i+1
                add_nonzero_block!(Is, Js, dyn_lag, lip1)
            else
                # [l_i, u_i, x_i+1, l_i+1]
                add_nonzero_block!(Is, Js, dyn_lag, dyn_lag, li, lip1)
            end
            # v
