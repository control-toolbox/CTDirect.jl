...
function runningCost(docp::DOCP{<: GenericIRK}, xu, v, time_grid)
    ...
    for i in 1:docp.time.steps
        ...
        # loop over stages
        for j in 1:docp.discretization.stage
            ...
            for l in 1:docp.discretization.stage
                kil = get_stagevars_at_time_step(xu, docp, i, l)
                @views @. work_xij[1:docp.dims.OCP_x] =
                    work_xij[1:docp.dims.OCP_x] +
                    hi * docp.discretization.butcher_a[j, l] * kil[1:docp.dims.OCP_x]
            end
            ...
            if j == 1
                work_sumbk[1] =
                    docp.discretization.butcher_b[j] *
...
function setStepConstraints!(docp::DOCP{<: GenericIRK}, c, xu, v, time_grid, i, work)
    ...
    if i <= docp.time.steps
        ...
        for j in 1:docp.discretization.stage
            ...
            # split to avoid dual tag ordering error in AD
            if j == 1
                @views @. work_sumbk[1:docp.dims.NLP_x] =
                    docp.discretization.butcher_b[j] * kij[1:docp.dims.NLP_x]
            else
                @views @. work_sumbk[1:docp.dims.NLP_x] =
                    work_sumbk[1:docp.dims.NLP_x] +
                    docp.discretization.butcher_b[j] * kij[1:docp.dims.NLP_x]
            end
            ...
            CTModels.dynamics(docp.ocp)(
                (@view c[(offset + offset_stage_eqs + 1):(offset + offset_stage_eqs + docp.dims.OCP_x)]),
                tij,
                work_xij,
                ui,
                v,
            ...
            if docp.flags.lagrange && docp.flags.lagrange_to_mayer
                c[offset + offset_stage_eqs + docp.dims.NLP_x] = CTModels.lagrange(docp.ocp)(
                    tij, work_xij, ui, v
                )
            end
            @views @. c[(offset + offset_stage_eqs + 1):(offset + offset_stage_eqs + docp.dims.NLP_x)] =
                kij -
                c[(offset + offset_stage_eqs + 1):(offset + offset_stage_eqs + docp.dims.NLP_x)]
            offset_stage_eqs += docp.dims.NLP_x
        end

        # state equation x_i+1 = x_i + h_i sum b_j k_i^j
        @views @. c[(offset + 1):(offset + docp.dims.OCP_x)] =
            xip1 - (xi + hi * work_sumbk[1:docp.dims.OCP_x])
        if docp.flags.lagrange && docp.flags.lagrange_to_mayer
            c[offset + docp.dims.NLP_x] =
                get_lagrange_state_at_time_step(xu, docp, i+1) - (
                    get_lagrange_state_at_time_step(xu, docp, i) +
                    hi * work_sumbk[docp.dims.NLP_x]
                )
        end
...
function DOCP_Jacobian_pattern(docp::DOCP{<: GenericIRK})
    ...
    for i in 1:docp.time.steps
        ...
        add_nonzero_block!(
            Is,
            Js,
            c_offset+docp.dims.NLP_x+1,
            c_offset+(s+1)*docp.dims.NLP_x,
            xi_start,
            xi_end,
        )
        add_nonzero_block!(
        ...
        add_nonzero_block!(
            Is, Js, c_offset+(s+1)*docp.dims.NLP_x+1, c_offset+c_block, xi_start, xi_end
        )
        add_nonzero_block!(
            Is, Js, c_offset+(s+1)*docp.dims.NLP_x+1, c_offset+c_block, ui_start, ui_end
        )
        add_nonzero_block!(
            Is, Js, c_offset+(s+1)*docp.dims.NLP_x+1, c_offset+c_block, v_start, v_end
        )
    end
    ...
    uf_end =
        var_offset - docp.discretization._step_variables_block +
        docp.dims.NLP_x +
        docp.dims.NLP_u
