...
@testset verbose = true showtiming = true ":constant_xv" begin
    sol = solve(ocp, display=false, init=(state=x_const, variable=v_const), max_iter=maxiter)
    @test(check_xf(sol, x_const) && check_v(sol, v_const))
end
@testset verbose = true showtiming = true ":constant_uv" begin
    sol = solve(ocp, display=false, init=(control=u_const, variable=v_const), max_iter=maxiter)
    @test(check_uf(sol, u_const) && check_v(sol, v_const))
end
@testset verbose = true showtiming = true ":constant_xuv" begin
    sol = solve(ocp, display=false, init=(state=x_const, control=u_const, variable=v_const), max_iter=maxiter)
...
@testset verbose = true showtiming = true ":vector_txu :constant_v" begin
    sol = solve(ocp, display=false, init=(time=t_vec, state=x_vec, control=u_vec, variable=v_const), max_iter=maxiter)
...
@testset verbose = true showtiming = true ":matrix_t :vector_xu :constant_v" begin
    sol = solve(ocp, display=false, init=(time=t_matrix, state=x_vec, control=u_vec, variable=v_const), max_iter=maxiter)
    @test(check_xf(sol, x_vec[end]) && check_uf(sol, u_vec[end]) && check_v(sol, v_const))
end
@testset verbose = true showtiming = true ":matrix_x :vector_tu :constant_v" begin
    sol = solve(ocp, display=false, init=(time=t_vec, state=x_matrix, control=u_vec, variable=v_const), max_iter=maxiter)
    @test(check_xf(sol, x_vec[end]) && check_uf(sol, u_vec[end]) && check_v(sol, v_const))
end

# 1.e mixed initial guess
@testset verbose = true showtiming = true ":vector_tx :functional_u :constant_v" begin
    sol = solve(ocp, display=false, init=(time=t_vec, state=x_vec, control=u_func, variable=v_const), max_iter=maxiter)
