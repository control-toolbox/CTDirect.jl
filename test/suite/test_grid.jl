...
@testset verbose = true showtiming = true ":explicit_grid" begin
    sol = solve(ocp, time_grid=LinRange(0,1,CTDirect.__grid_size_direct()+1), print_level=0)
    @test (sol.objective == sol0.objective) && (sol.iterations == sol0.iterations)
end
...
@testset verbose = true showtiming = true ":explicit_grid" begin
    sol = solve(ocpT2, time_grid=LinRange(0,1,CTDirect.__grid_size_direct()+1),print_level=0)
    @test (sol.objective == sol0.objective) && (sol.iterations == sol0.iterations)
end
